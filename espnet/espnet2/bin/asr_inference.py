...
class Speech2Text:
    ...
    def __call__(
        ...
        acoustic_feat: Optional[torch.Tensor] = None,
    ) -> Union[
    # def __call__(self, speech: Union[torch.Tensor, np.ndarray]) -> Union[
        ...
        if acoustic_feat is not None:
            if isinstance(acoustic_feat, np.ndarray):
                acoustic_feat = torch.tensor(acoustic_feat)

            acoustic_feat = acoustic_feat.unsqueeze(0).to(getattr(torch, self.dtype))
            batch["acoustic_feat"] = acoustic_feat
